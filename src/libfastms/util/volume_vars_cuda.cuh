...
__device__ __forceinline__ int cuda_z() { return threadIdx.z + blockDim.z * blockIdx.z; }
...
inline dim3 cuda_block_size(int w, int h, int d)
{
	dim3 block(128, 2, 1);
	block.x = std::min((int)block.x, w);
	block.y = std::min((int)block.y, h);
    block.z = std::min((int)block.z, d);
	return block;
}


inline dim3 cuda_grid_size(dim3 block, int w, int h, int d)
{
	return dim3(
            (w + block.x - 1) / block.x, 
            (h + block.y - 1) / block.y, 
